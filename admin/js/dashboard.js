...
function showAddModal() {
    ...
    delete document.getElementById('sahabiForm').dataset.editId;
...
function editSahabi(id) {
    ...
    if (sahabi) {
        ...
        Array.from(categorySelect.options).forEach(option => {
            option.selected = sahabi.categories && sahabi.categories.includes(option.value);
        });
        
        // Store the ID for updating
        document.getElementById('sahabiForm').dataset.editId = id;
        document.getElementById('addEditModal').style.display = 'block';
    }
}
...
async function saveSahabi(formData) {
    try {
        const editId = document.getElementById('sahabiForm').dataset.editId;
        const url = editId ? `/api/sahaba/${editId}` : '/api/sahaba';
